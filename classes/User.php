...
require_once "Database.php";    // using require,  if an error occurs, the process would not proceed
...
class User extends Database
{
    ...
    public function store($request)
    {
        ...
        if ($this->conn->query($sql)){
            header('location: ../views');   // go to index.php (login)  //password: kredo1234
            exit;
        }
        else {
    ...
    public function login($request)
    {
        ...
        if ($result->num_rows == 1){
            ...
            // $user = ['id' => 1, 'usrname' => 'james', 'password' => 'hogehoge'];
    ...
    public function getUser()
    {
        ...
        if ($result = $this->conn->query($sql)){
            ...
            die('Error retriefing the user: ' . $this->conn->error);
        }
    }
    // endgetUser
    ...
    public function update($request, $files)
    {
        ...
        if ($this->conn->query($sql)){
            ...
            if ($photo){
                ...
                if ($this->conn->query($sql)){
                    // Save the actual image to imeges folder
